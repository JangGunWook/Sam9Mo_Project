...
def daily_save():  
    ...
    for code in KOSPI_STOCK_CODE:
        time.sleep(0.5)
        collection = db["kospi_daily_stock"]
        data={
            "_id" : code
        }
        try:
        ...
        try:
            stock= get_domestic_stock_price(code, yesterday,yesterday)["output2"]
            print(f"kospi {today}일에 save ")
            # 배열의 맨위에 추가된다!
    ...
    for code in KOSDAQ_STOCK_CODE:
        time.sleep(0.5)
        collection = db["kosdaq_daily_stock"]
        data={
            "_id" : code
        }
        try:

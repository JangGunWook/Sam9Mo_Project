...
def week_save():
    ...
    def find_last_monday():
        ...
        print("today: ", today)
        if today == 0:
            last_week_monday = datetime.today() - timedelta(days=6)  # 전 주 월요일 계산
            
            return int(last_week_monday.strftime("%Y%m%d"))
        else:
            print("금일은 월요일이 아닙니다.")

    last_tuesday=find_last_monday()
    print(last_tuesday)
    ...
    # kospi 종목 40 주봉데이터 매주 월요일에 저장
    if datetime.today().weekday() == 0: # 월요일인지 체크
        for code in KOSPI_STOCK_CODE:
            time.sleep(0.5)
            collection = db["kospi_week_stock"]
            data={
                "_id" : code
            }
            try:
                ...
                print("현재 있는 키입니다", e)
            try:
                stock= get_domestic_stock_price(code, last_tuesday,today)["output2"]
                # 배열의 맨위에 추가된다!
    ...
    # kosdaq 종목 40 주봉데이터 매주 월요일에 저장
    if datetime.today().weekday() == 0: # 월요일인지 체크
        for code in KOSDAQ_STOCK_CODE:
            time.sleep(0.5)
            collection = db["kosdaq_week_stock"]
            data={
                "_id" : code
            }
            try:
